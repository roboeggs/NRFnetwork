...
void setup_time() {
  ...
  do{
     ...
     if (enc.getEncoder(ENC_TURN_RIGHT) and h < 23){
      ++h;
      display_time(h, m);
     }
     //if (enc1.isLeft() and h > 0){
     if (enc.getEncoder(ENC_TURN_LEFT) and h > 0){
      --h;
      display_time(h, m);
     }
  }while(!enc.getButton(KEY_PUSHED));                                       //пока кнопка не нажата 
  matrix.fillScreen(LOW);                                            // очищаем матрицу
  display_time(h, m);
  do{
     question();
     ...
     if (enc.getEncoder(ENC_TURN_RIGHT) and m < 59){
      ++m;
      display_time(h, m);
     }
     if (enc.getEncoder(ENC_TURN_LEFT) and m > 0){
      --m;
      display_time(h, m);
     }
  }while(!enc.getButton(KEY_PUSHED));
